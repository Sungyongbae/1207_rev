...
def get_RSI(ticker, period, num, column='close'):
...
#ticker = result_get[0]
#print(get_RSI(ticker,10,-1))
#print(get_RSI(ticker,10,-2))

# 로그인
#upbit = pyupbit.Upbit(access, secret)
bot.sendMessage(ID, '========start here_211118========')
#trade_ticker = 'STORJ'
#ticker = 'KRW-STORJ'
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=8):
            check_inform = False
            check_add_buy = False
            first_money = 100000
            buy_money = 100000
            if n == 0 and first_buy == False:
                ...
                check_RSI = get_RSI(ticker,10,-1)
                check_RSI_again = get_RSI(ticker,10,-2)

                if check_RSI>70 and check_RSI_again>70:
                    bot.sendMessage(ID, str(trade_ticker) + '\n'
                            + "이전 RSI: " + str(check_RSI_again) + '\n'
                            + "현재 RSI: " + str(check_RSI) + '\n'
                            + "RSI 과열로 첫매수 중단")
                else:
                    current_price = get_current_price(ticker)
                ...
                if n < 19:
                    ...
                    bot.sendMessage(ID, ">>>> SELL_failure <<<<" + '\n'
                                    ...
                                    + "매도가격: " + str(sell_price) + '\n'
                                    + "매도금액: " + str(my_money) + '\n'
                                    + "손실률: " + str(current_profit)+ '\n'
                                    + "test...ing")
                    n=0
                    first_buy = False
                    sell_all = True
                    
                if current_profit >=k and sell_all == False and n<44:
                    print('수익 매도 시작')
                    sell_price = get_current_price(ticker)
                    my_money = sell_price*(possess_total*0.9995)
                    success += 1
            ...
            current_price = get_current_price(ticker)
            current_rsi = get_RSI(ticker,10,-1)
